...
Parser._parseAToB = (abMap, a, fallback = undefined) => {
...
Parser.getFullSource = (source) => {
    if (source && source.source)
        return Parser._parseAToB(sources_1.OFFICIAL_SOURCES, source.source);
    return Parser._parseAToB(sources_1.OFFICIAL_SOURCES, source);
};
Parser.getCreatureType = (creatureType) => {
...
Parser.crToXp = (cr, { isDouble = false } = {}) => {
    if (cr != null && cr.xp)
...
Parser.sizeAbvToFull = (abv) => {
    if (_.isEmpty(abv))
        return 'unknown';
    return Parser._parseAToB(size_1.SIZE_ABV_TO_FULL, abv, undefined);
};
...
Parser.senseToExplanation = (senseType) => {
    ...
    return Parser._parseAToB(senses_1.SENSE_JSON_TO_FULL, senseType, ['No explanation available.']);
};
...
Parser.spDurationToFull = (dur) => {
    ...
    const outParts = dur.map((d) => {
        switch (d.type) {
            ...
            case 'permanent': {
                if (d.ends) {
                    const endsToJoin = d.ends.map((m) => Parser._parseAToB(duration_1.SP_END_TYPE_TO_FULL, m));
...
Parser.spTime = (time) => {
    const text = time.map((timeItem) => {
        return {
            full: `${timeItem.number} ${!_.isEmpty(timeItem.unit) ? Parser._parseAToB(time_1.SP_TIME_TO_FULL, timeItem.unit) : ''}`,
            short: `${timeItem.number} ${!_.isEmpty(timeItem.unit) ? Parser._parseAToB(time_1.SP_TIME_TO_ABV, timeItem.unit) : ''}`,
        };
    });
    return {
        text,
...
Parser.spSchool = (school) => {
    return Parser._parseAToB(schools_1.SP_SCHOOL_ABV_TO_FULL, school);
};
